...
const TARGET_URL = 'http://localhost:4000/api/v1/posts';
const CONCURRENT_REQUESTS = 3; // 3 RPS controlled test
...
class ControlledTest {
    ...
    return new Promise((resolve) => {
      ...
      const req = http.get(TARGET_URL, (res) => {
        ...
        res.on('end', () => {
          ...
          // Parse response to get performance data
          let queryTime = null;
          try {
            const parsed = JSON.parse(data);
            if (parsed.performance) {
              queryTime = parseFloat(parsed.performance.queryTime);
            }
          } catch (e) {
            // Ignore parse errors
          }
  ...
  async runTest() {
    console.log(`🎯 CONTROLLED TEST: ${CONCURRENT_REQUESTS} RPS for ${TEST_DURATION/1000}s`);
  ...
  analyzeResults() {
    ...
    const queryTimes = successfulRequests.filter(r => r.queryTime !== null).map(r => r.queryTime);
    ...
    console.log(`🗄️  Avg DB Query Time: ${avgQueryTime.toFixed(2)}ms`);
    console.log(`⚙️  Avg Total Time: ${avgTotalTime.toFixed(2)}ms`);
    
    if (avgQueryTime > 0) {
      const overhead = avgTotalTime - avgQueryTime;
      console.log(`📊 System Overhead: ${overhead.toFixed(2)}ms (${(overhead/avgTotalTime*100).toFixed(1)}%)`);
    }
  }
}

// Run controlled test
const tester = new ControlledTest();
